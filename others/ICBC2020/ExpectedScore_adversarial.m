function [expectedScoreSt,expectedScoreSf, PrTrueAsMajority,PrFalseAsMajority,expectedScoreAdv] = ExpectedScore_adversarial(PrT1,PrStT1,PrStT2,PrTrue,n,p)
...
% signal probability
if n*(1-p) > n/2
    PrTrueAsMajority = 1-(binocdf(n/2, n*(1-p), PrTrue));
else
    PrTrueAsMajority = 0;
end
if n*p<n/2
    PrFalseAsMajority = 1-(binocdf(n/2-n*p, n*(1-p), PrFalse));
else 
    PrFalseAsMajority = 1;
end

PrFalse = PrFalse * (1-p) + p*1;
PrTrue = 1-PrFalse;
...
expectedScoreSf = ... % information score
                  ...
                  ... % prediction score
                      ...
                      + (1 - PrTrueAsMajority - PrFalseAsMajority) * ...% if no majority, pick another voter as reference
                            (PrTrue * quadraticScoringRule(1,ySt) ...
                            + PrFalse * quadraticScoringRule(0,ySt)));
expectedScoreAdv = ... % information score
                      (PrTrue * (PrTrue * quadraticScoringRule(1,findYPrim(0,ySt)) + PrFalse * quadraticScoringRule(0,findYPrim(0,ySt))) ...
                         + PrFalse * (PrTrue * quadraticScoringRule(1,findYPrim(0,ySf)) + PrFalse * quadraticScoringRule(0,findYPrim(0,ySf)))) ...
                  ... % prediction score
                      + 1;
